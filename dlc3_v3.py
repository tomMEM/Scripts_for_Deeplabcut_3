...
def run_interactive_training():
    ...
    # --- Safety cleanup before resuming ---
    snapshot_dir = Path(train_folder_path)
    for existing_best in snapshot_dir.glob("snapshot-best-*.pt"):
        safe_name = existing_best.with_name(existing_best.name.replace("snapshot-best-", "snapshot-keep-best-"))
        existing_best.rename(safe_name)
        print(f"🧩 Renamed {existing_best.name} → {safe_name.name} to avoid rename conflict.")
